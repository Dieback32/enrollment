...
<?php if ($this->session->userdata('pay_id') != NULL): ?>
	...
	$cash = $infoss->row();
	if ($this->session->userdata('payAmount') > 0) {
		$change = $this->session->userdata('payAmount') - $ids->amount;
	} else {
		$change = 0;
	};
	...
	<div id="printableArea">
		...
		<table class="table table-hover">
			...
			<tr>
				<td>Payment:</td>
				<td><?php echo stringToFloat($ids->amount) ?></td>
			</tr>
			<tr>
				<td>Paid Amount:</td>
				<td><?php echo stringToFloat($this->session->userdata('payAmount')) ?></td>
			</tr>
			<tr>
				<td>Change:</td>
				<td><?php echo stringToFloat($change) ?></td>
			</tr>
			</tbody>
		</table>
	</div>
...
<div class="text-center">
	<p><u><strong><?php if (@$cash->lastName) {
					echo $cash->lastName . ' ' . $cash->firstName;
				} else {
					echo 'Loading...';
				} ?></strong></u></p>
	<p>Cashier</p>
...
<br><br>
<center><input type="button" class="btn btn-primary" onclick="printDiv('printableArea')" value="Print Receipt"/>
</center>
